/* Compiled by kdc on Fri Aug 29 2014 18:26:27 GMT+0000 (UTC) */
(function() {
/* KDAPP STARTS */
...
/* BLOCK STARTS: /home/axchistyakov/Applications/Gitdashboard.kdapp/controller/repodatamanager.coffee */
var RepoDataManager,
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

RepoDataManager = (function() {
  function RepoDataManager(options, data) {
    if (options == null) {
      options = {};
    }
    ...
    this.checkDataPath = __bind(this.checkDataPath, this);
    this.verifyRepoData = __bind(this.verifyRepoData, this);
    this.getRepoDirectory = __bind(this.getRepoDirectory, this);
    this.listRepository = __bind(this.listRepository, this);
    this.unlistRepository = __bind(this.unlistRepository, this);
  ...
  RepoDataManager.prototype.readRepoData = function() {
    var _this = this;
    return this.kiteHelper.run({
      command: "cat " + dataPath
    }).then(function(readData) {
      var data, dataLine, i, _i, _len, _results;
      data = readData.stdout.split("\n");
      _results = [];
      for (_i = 0, _len = data.length; _i < _len; _i++) {
        dataLine = data[_i];
        i = dataLine.indexOf(" ");
        _results.push(_this.repodata[dataLine.substring(0, i)] = dataLine.substring(i + 1));
      }
      return _results;
    });
  };

  RepoDataManager.prototype.repositoryIsListed = function(name) {
    if (this.repodata == null) {
      return false;
    } else if (this.repodata[name] != null) {
      return true;
    } else {
      return false;
    }
  ...
  RepoDataManager.prototype.gitPresentInRepoFolder = function(dir) {
    var _this = this;
    return this.kiteHelper.run({
  ...
  RepoDataManager.prototype.unlistRepository = function(name, path) {
    var _this = this;
    if (this.repodata[name] != null) {
      delete this.repodata[name];
      return this.kiteHelper.run({
  ...
  RepoDataManager.prototype.getRepoDirectory = function(name) {
    return this.repodata[name];
  };

  RepoDataManager.prototype.verifyRepoData = function() {
    var keys,
      _this = this;
    keys = Object.keys(this.repodata);
    return Promise.all(keys.map(function(key) {
      return _this.gitPresentInRepoFolder(_this.repodata[key].then(function(present) {
        if (!present) {
          return key;
        }
      }));
  ...
  RepoDataManager.prototype.checkDataPath = function() {
    var _this = this;
    return this.kiteHelper.getKite().then(function(kite) {
      return kite.fsExists({
        path: dataPath
      }).then(function(exists) {
        return _this.directoryExists = exists;
      });
    });
  };
  ...
  RepoDataManager.prototype.checkSSHKeys = function() {
    var _this = this;
    return this.kiteHelper.run({
  ...
  RepoDataManager.prototype.generateSSHKeys = function(email, passphrase) {
    if (passphrase == null) {
      passphrase = "";
    }
    return this.kiteHelper.run({
  ...
  RepoDataManager.prototype.compareSSHKeys = function() {
    var _this = this;
    return this.readSSHKeys().then(function(key) {
...
GitdashboardCloneModal = (function(_super) {
  ...
  function GitdashboardCloneModal(options, data) {
    ...
    this.dataManager.kiteHelper.getKite().then(function(kite) {
      _this.unmountAll();
      _this.finderController.mountVm(_this.dataManager.kiteHelper.getVmByName(_this.dataManager.kiteHelper.getVm()));
  ...
  GitdashboardCloneModal.prototype.beginClone = function() {
    ...
    this.destroy();
    return this.dataManager.cloneRepo(this.repoView.getOptions().name, this.repoView.getOptions().cloneUrl, fullPath).then(function(cloned) {
      if (!cloned) {
  ...
  GitdashboardCloneModal.prototype.unmountAll = function() {
    ...
    _ref = this.dataManager.kiteHelper._vms;
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      vmR = _ref[_i];
      _results.push(console.log(this.finderController.unmountVm(vmR.hostnameAlias)));
    }
    return _results;
  };
...
RepoView = (function(_super) {
  ...
  function RepoView(options, data) {
    ...
    options.url || (options.url = "");
    options.sshCloneUrl || (options.sshCloneUrl = null);
  ...
  RepoView.prototype.cloneToMachine = function() {
    return new GitdashboardCloneModal({
      ...
      dataManager: this.controller.dataManager
    });
  };
  ...
  RepoView.prototype.updateState = function() {
    ...
    if (this.controller.dataManager.repositoryIsListed(name)) {
      this.state = CLONED;
      this.openDir = this.controller.dataManager.getRepoDirectory(name);
    } else {
      this.state = NOT_CLONED;
    }
  ...
  RepoView.prototype.updateView = function() {
    ...
    this.cloneButton.unsetClass("state-cloned");
    this.cloneButton.unsetClass("state-uncloned");
    this.cloneButton.unsetClass("state-cloning");
    this.cloneButton.unsetClass("state-loading");
    this.cloneButton.setClass("clone-button");
    this.cloneButton.setCallback(void 0);
    this.cloneButton.hideLoader();
    if (this.state === CLONED) {
      this.cloneButton.setClass("state-cloned");
      this.cloneButton.setTitle("Open");
      return this.cloneButton.setCallback(function() {
      ...
      return this.cloneButton.setClass("state-uncloned");
    } else if (this.state === CLONING) {
      this.cloneButton.disable();
      this.cloneButton.setClass("state-cloning");
      return this.cloneButton.setTitle("Cloning...");
    } else if (this.state === LOADING) {
      this.cloneButton.setClass("state-loading");
  ...
  RepoView.prototype.writeInstalled = function(path) {
    var _this = this;
    return this.controller.dataManager.listRepository(this.getOptions().name, path.then(function() {
...
RepoDataController = (function(_super) {
  ...
  function RepoDataController(options, data) {
    ...
    repoDataController = KD.singletons.repoDataController;
    if (repoDataController) {
      return repoDataController;
    }
    RepoDataController.__super__.constructor.call(this, options, data);
    this.dataManager = options.dataManager;
    this.registerSingleton("repoDataController", this, true);
    this.dataManager.checkDataPath().then(function(exists) {
      if (exists) {
        return _this.dataManager.readRepoData().then(function() {
          return _this.dataManager.verifyRepoData().then(function() {
  ...
  RepoDataController.prototype.getSearchedRepos = function(callback, topic) {
    ...
    return $.getJSON(link).then(function(json) {
      ...
      for (i = _i = 0; 0 <= searchResultCount ? _i < searchResultCount : _i > searchResultCount; i = 0 <= searchResultCount ? ++_i : --_i) {
        ...
        _this.appendExtras(options);
        callback(new RepoView(options));
      }
      return _this.emit("search-results-downloaded");
    }).fail(function(err) {
  ...
  RepoDataController.prototype.getTrendingRepos = function(callback) {
    var _this = this;
    return this.appStorage.fetchStorage(function() {
...
/* BLOCK STARTS: /home/axchistyakov/Applications/Gitdashboard.kdapp/views/searchpaneview.coffee */
var GitdashboardSearchPaneView,
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

GitdashboardSearchPaneView = (function(_super) {
  __extends(GitdashboardSearchPaneView, _super);

  function GitdashboardSearchPaneView(options, data) {
    if (options == null) {
      options = {};
    }
    ...
    GitdashboardSearchPaneView.__super__.constructor.call(this, options, data);
    this.controller = options.controller;
    this.controller.on("search-results-downloaded", this.bound("hideLoader"));
    this.loading = false;
  }

  GitdashboardSearchPaneView.prototype.viewAppended = function() {
    var _this = this;
    ...
    this.searchBox.on('keydown', function(e) {
      var loading;
      if (e.keyCode === 13) {
        if (!loading || !Boolean(_this.searchBox.getValue())) {
          loading = true;
          _this.loader.show();
    ...
    return this.addSubView(this.container = new KDListView({
      cssClass: "container"
    }));
  };

  GitdashboardSearchPaneView.prototype.repoReceived = function(repoView) {
    var loading;
    if (repoView === false) {
      this.container.empty();
      this.hideLoader();
      this.container.addItemView(new KDView({
  ...
  GitdashboardSearchPaneView.prototype.hideLoader = function() {
    var loading;
    loading = false;
    return this.loader.hide();
  };

  return GitdashboardSearchPaneView;
...
GitDashboardTrendingPageView = (function(_super) {
  ...
  function GitDashboardTrendingPageView(options, data) {
    ...
    this.controller.on("trending-page-downloaded", this.bound("hideLoader"));
  }
  ...
  GitDashboardTrendingPageView.prototype.hideLoader = function() {
    return this.loader.hide();
  };
...
GitDashboardMainView = (function(_super) {
  ...
  GitDashboardMainView.prototype.viewAppended = function() {
    ...
    this.tabView.addPane(this.searchPagePane = new KDTabPaneView({
      title: "Search",
      closable: false
    }));
    this.searchPagePane.setMainView(new GitdashboardSearchPaneView({
      controller: this.controller
    }));
    this.trendingPagePane.setMainView(new GitDashboardTrendingPageView({
      dataController: this.controller
    }));
    this.tabView.showPane(this.trendingPagePane);
  ...
  GitDashboardMainView.prototype.initPersonal = function(token) {
    ...
    return this.dataManager.checkSSHKeys().then(function(exist) {
      var container, modal;
      if (!exist) {
